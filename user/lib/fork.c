...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if (!(perm & PTE_COW)) {
		user_panic("cow_entry: page at 0x%x does not contain the PTE_COW flag", va);
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	if (!(perm & PTE_D) || (perm & PTE_LIBRARY) || (perm & PTE_POOL)) {
		// Read-Only or Shared page
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
	} else {
		// If it is COW page, then mapping it and setting perm is Okay
		// Otherwise, set the PTE_D to 0 and PTE_COW to 1
		perm = (perm | PTE_COW) & ~PTE_D;
		syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);  // Map to the child first
...
int fork(void) {
	...
	u_int pd_high_bound = PDX(ROUND(USTACKTOP, PDMAP));
	u_int vpn_high_bound = VPN(USTACKTOP);
	for (i = 0; i < pd_high_bound; ++i) {
		if (!(vpd[i] & PTE_V)) {
			continue;
		}
		for (j = 0; j < 1024; ++j) {
			u_int vpn = (i << 10) | j;
			if (vpn < vpn_high_bound && (vpt[vpn] & PTE_V)) {
				duppage(child, vpn);
			}
		}
	}
