...
void vprintfmt(fmt_callback_t out, void *data, const char *fmt, va_list ap) {
	...
	for (;;) {
		...
		// Yeah! We found a '%', which is *fmt;
		++fmt; // Now we consume the '%'
		...
		case 'D':
			...
			if (num < 0) {
				// assume that num is 32-bit width
				// if (num == -2147483648 == 0x80000000), then (1 + ~num == 0x80000000)
				// as unsigned, num == 2147483648 actually
				num = 1 + ~num;
				neg_flag = 1;
			}
			print_num(out, data, /* unsigned long */num, 10, neg_flag, width, ladjust, padc, 0);
			break;
